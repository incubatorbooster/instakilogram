...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3522
        NAMESPACE = "content-universe-store"
        REGISTRY_HOSTNAME = "contentuniverse"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "instakilogram"
        DEPLOYMENT_NAME = "instakilogram-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'content-universe-store') {
